...
export function Task({
  ...
  return (
    <div className="flex items-center justify-between bg-white border-t-2 border-gray-200 px-3 lg:px-5 py-3">
      ...
      <div className="flex flex-col md:flex-row items-center gap-2 md:gap-4 shrink-0">
        <button className="cursor-pointer" onClick={() => setEdit(id)}>
          <img className="w-4 md:w-5 lg:w-6" src={editIcon} />
        </button>

        <button className="cursor-pointer" onClick={() => deleteTask(id)}>
          <img className="w-4 md:w-5 lg:w-6" src={deleteIcon} />
        </button>
      </div>
    </div>
  );
